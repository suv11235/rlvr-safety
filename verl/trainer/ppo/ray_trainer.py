            ...
            for step_index, batch_dict in enumerate(self.train_dataloader):
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        try:
                            reward_result = self.reward_fn(batch, step_index=step_index if self.config.trainer.defence_mode else None, return_dict=True)
                            reward_tensor = reward_result['reward_tensor']
                            reward_extra_infos_dict = reward_result['reward_extra_info']
                        except Exception as e:
                            print(f'Error in reward_fn: {e}')
                            reward_tensor = self.reward_fn(batch, step_index=step_index if self.config.trainer.defence_mode else None)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            if self.config.trainer.defence_mode and step_index % 2 == 0 and step_index >=25:
